...
ifeq ($(OS_DET),WIN)
	...
	SHARED_LIBRARY_HEADER_CMD = type $(LIBRARY_SRC_DIR)\mingw_threads.h >> $(PROGRAM_CPP_APP_HEADER_BACKSLASH) 2>NUL & \
		type $(LIBRARY_SRC_DIR)\ResourceFile.h >> $(PROGRAM_CPP_APP_HEADER_BACKSLASH) 2>NUL & \
...
all: directories $(SHARED_CPP_PATH) $(STATIC_CPP_PATH) $(PROGRAM_CPP_EXE) $(PROGRAM_CSHARP_EXE) $(PROGRAM_CPP_APP_ENABLE)
