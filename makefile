...
ifeq ($(OS_DET),LINUX)
	...
	COPY_SHARED_LIBRARY_CMD = 
	LIBRARY_TEMP_DIR_DEL = yes | rmdir $(LIBRARY_TEMP_DIR)
	PROGRAM_OBJ_DIR_DEL = yes | rmdir $(PROGRAM_OBJ_DIR)
	SHARED_LIBRARY_DEL_CMD = yes | rm "$(SHARED_CPP_PATH)"
	STATIC_LIBRARY_DEL_CMD = yes | rm "$(STATIC_CPP_PATH)"
	PROGRAM_CSHARP_DEL_CMD = yes | rm "$(PROGRAM_CSHARP_EXE)"
	PROGRAM_CSHARP_DEL_POST_CMD = $(PROGRAM_CSHARP_DEL_CMD)
	BUNDLE_CSHARP_DEL_CMD = yes | rm "$(PROGRAM_CSHARP_BUNDLE)"
	PROGRAM_CPP_DEL_CMD = yes | rm "$(PROGRAM_CPP_EXE)"
endif
ifeq ($(OS_DET),android)
	# paths
	SHARED_CPP_NAME = libResourceFileUtility.dll
