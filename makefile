...
ifeq ($(OS_DET),LINUX)
	...
	COPY_SHARED_LIBRARY_CMD_UNUSED = yes | cp -rf $(SHARED_CPP_PATH) $(PROGRAM_BIN_DIR)
	COPY_SHARED_LIBRARY_CMD = 
	LIBRARY_TEMP_DIR_DEL = yes | rmdir -rf $(LIBRARY_TEMP_DIR)
	PROGRAM_OBJ_DIR_DEL = yes | rmdir -rf $(PROGRAM_OBJ_DIR)
	SHARED_LIBRARY_DEL_CMD = yes | rmdir -f "$(SHARED_CPP_PATH)"
	STATIC_LIBRARY_DEL_CMD = yes | rmdir -f "$(STATIC_CPP_PATH)"
	PROGRAM_CSHARP_DEL_CMD = yes | rmdir -f "$(PROGRAM_CSHARP_EXE)"
	PROGRAM_CSHARP_DEL_POST_CMD = $(PROGRAM_CSHARP_DEL_CMD)
	BUNDLE_CSHARP_DEL_CMD = yes | rm -f "$(PROGRAM_CSHARP_BUNDLE)"
	PROGRAM_CPP_DEL_CMD = yes | rm -f "$(PROGRAM_CPP_EXE)"
endif
